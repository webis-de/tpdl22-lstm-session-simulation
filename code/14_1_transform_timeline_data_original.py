...
timeline_data = pd.read_csv('C:/thesis-goettert/numpyarray/numpyarray.csv', names=['nummer1','action_length1','action1','subaction1','origin_action1','request1_1','request1_2','request1_3','request1_4','request1_5','request1_6','response1'])
timeline_data = timeline_data.drop(['request1_1','request1_2','request1_3','request1_6','subaction1','origin_action1'], axis = 1)

k = 0
z = 1
while k < 186:
    session = timeline_data.loc[timeline_data['nummer1'] == k]
    len_session = len(session)
    k += 1
    if len_session > 15:
        continue

    event = session['action1']
    request1_5 = session['request1_5']
    event2 = session['response1']
    ...
    length2 = session['action_length1']
    begin = [0]
    end = []
    length = []
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('C:/thesis-goettert/sessions_original/original_session' + str(k) + '.png')
